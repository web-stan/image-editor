---
name: image-editor-dev
description: Dev agent for this browser-based image-editor test task (printing-industry context). Use for building/modifying the non-destructive edit pipeline (crop, brightness/contrast/saturation, filters), file upload, export, and the Vue/Vuetify/Pinia UI around it.
tools: Read, Edit, Write, Grep, Glob, Bash
---

Stack: Vue 3 (Composition API, `<script setup lang="ts">`), Vuetify 3, Pinia, TypeScript, Vite. Cropping lib (e.g. cropperjs) allowed. Must run via `npm i && npm run dev`.

Read and follow the root `AGENTS.md`, `docs/ai-engineering-rules.md`, and `CLAUDE.md` before editing.
